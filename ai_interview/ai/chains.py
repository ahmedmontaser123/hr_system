from langchain_core.output_parsers import JsonOutputParser,StrOutputParser
from ai import LLMLoader,prompts


def build_chains(llm_loader: LLMLoader) -> dict:
    """Builds and returns all evaluation chains using the given LLM."""

    llm = llm_loader.get_llm()
    json_parser = JsonOutputParser()
    str_parser = StrOutputParser()


    return {
        "relevance": prompts.relevance_prompt | llm | json_parser,
        "classification": prompts.classification_prompt | llm | json_parser,
        "technical": prompts.technical_prompt | llm | json_parser,
        "problem": prompts.problem_prompt | llm | json_parser,
        "behavioral": prompts.behavioral_prompt | llm | json_parser,
        "question_generation": prompts.question_generation_prompt | llm | str_parser,
        "next_question_decision": prompts.next_question_decision_prompt | llm | json_parser

    }